...
$.widget( "ui.menubar", {
	...
	_create: function() {
		...
		this.menuItems = this.element.children( this.options.itemTag );
	...
	_initializeMenuItems: function() {
		...
		subMenus = this.menuItems.children( this.options.menuTag ).menu({
			position: {
				within: menubar.options.position.within
			},
			select: function( event, ui ) {
				ui.item.parents( menubar.options.menuTag + ".ui-menu:last" ).hide();
				menubar._close();
				ui.item.parents( ".ui-menubar-item" ).children().first().focus();
				menubar._trigger( "select", event, ui );
			},
			menus: menubar.options.menuTag
		})
        .hide()
        .attr({
            "aria-hidden": "true",
	...
	_hasSubMenu: function( menuItem ) {
		return $( menuItem ).children( this.options.menuTag ).length > 0;
	},
		...
		if ( menuItemHasSubMenu ) {
			...
			this._on( anItem, {
				click: function(event) {
                    // clicking on the menu title that has no submenus. Treat like a selection.
					if ( this.active ) {
						this._close();
					}
                    var ui = { item: anItem.parent() };
	...
	_mouseBehaviorForMenuItemWithSubmenu: function( event ) {
		...
		menu = $(event.target).parents( ".ui-menubar-item" ).children( this.options.menuTag );
	...
	_collapseActiveMenu: function() {
		...
		this.active
			...
			.closest( this.options.itemTag ).removeClass( "ui-state-active" );
	},
	...
	_open: function( event, menu ) {
		...
		if ( this.active && this.active.length &&
				this._hasSubMenu( this.active.closest( this.options.itemTag ) ) ) {
					this._collapseActiveMenu();
		}
